    ...
    public class ContactsVM : INotifyPropertyChanged
    {
        ...
        public async void CallContact(Contact callingContact)
        {
            if (callingContact != null)
            {
                ...
                bool isCall = await ContactsPage.DisplayAlert("Quieres llamar a la persona?", message, "Call", "Cancel");
                if (isCall)
                {
                    //Go to Contacts App
        ...
        {
            await App.Current.MainPage.Navigation.PushAsync(new ContactDetailPage());
        }
